...
export function operatorExecutionPlan(prompt = '', user = {}) {
  const value = String(prompt || '').trim();
  const campaigns = Array.isArray(user?.workspace?.campaigns) ? user.workspace.campaigns : [];
  if (/(?:draft|write|prepare|create|make).*(?:follow[ -]?up|reply)/i.test(value)) {
    return { type: 'create-followup-draft', label: 'Create follow-up draft', prompt: value, route: '#messages', requiresConfirmation: false, detail: 'Creates and saves a draft. Nothing is sent.' };
  }
  if (/(?:create|write|draft|make).*(?:post|caption|email|content|copy)/i.test(value)) {
    return { type: 'create-content-draft', label: 'Create content draft', prompt: value, route: '#content', requiresConfirmation: true, detail: 'Uses 1 AI generation and saves the result to Content Studio.' };
  }
  if (/\b(?:pause|stop|hold)\b.*\bcampaign\b/i.test(value)) {
    const active = campaigns.filter(item => item?.status === 'active');
    if (active.length === 1) return { type: 'pause-campaign', label: `Pause ${active[0].name || 'campaign'}`, prompt: value, targetId: active[0].id, route: '#ads', requiresConfirmation: true, detail: 'Changes this campaign’s Nitro status from active to paused.' };
  }
  if (/\b(?:resume|restart|reactivate)\b.*\bcampaign\b/i.test(value)) {
    const paused = campaigns.filter(item => item?.status === 'paused');
    if (paused.length === 1) return { type: 'resume-campaign', label: `Resume ${paused[0].name || 'campaign'}`, prompt: value, targetId: paused[0].id, route: '#ads', requiresConfirmation: true, detail: 'Changes this campaign’s Nitro status from paused to active.' };
  }
  return null;
}

export function cleanOperatorAnswer(value = '', max = 6000) {
  const text = String(value ?? '').trim();
  if (!text || /^(?:undefined|null|nan|\[object Object\])$/i.test(text)) return '';
  return text
...
export function operatorFallbackResponse(prompt = '', snapshot = {}, priorities = [], company = 'Your business') {
  const value = String(prompt).trim().toLowerCase();
  if (/^(hi|hello|hey|howdy|yo)(\s+(there|nitro))?[!?.\s]*$/.test(value)) {
    return 'Hey! What can I help you with?';
  }
  if (/ice\s*cream|gelato|flavou?r/.test(value)) {
    return 'Go with cookies and cream for a safe crowd-pleaser. If you want something richer, try cookie dough; if you want something lighter, strawberry is a great pick.';
  }
  const action = inferOperatorAction(prompt);
  const agent = operatorAgent(prompt);
  const top = priorities[0];
  const facts = [
    `${Number(snapshot.websites || 0)} website${Number(snapshot.websites || 0) === 1 ? '' : 's'}`,
    `${Number(snapshot.content || 0)} content asset${Number(snapshot.content || 0) === 1 ? '' : 's'}`,
    `${Number(snapshot.socialScheduled || 0)} scheduled post${Number(snapshot.socialScheduled || 0) === 1 ? '' : 's'}`,
    `${Number(snapshot.connectedSocials || 0)}/5 social channels connected`,
    `${Number(snapshot.activeCampaigns || 0)} active campaign${Number(snapshot.activeCampaigns || 0) === 1 ? '' : 's'}`,
    `${Number(snapshot.replies || 0)} repl${Number(snapshot.replies || 0) === 1 ? 'y' : 'ies'}`,
  ];

  const businessQuestion = action || /(business|brief|workspace|priority|focus|next move|attention|status|nitro|growth|marketing)/.test(value);
  if (!businessQuestion) return 'I can answer everyday questions too, but my AI connection is temporarily unavailable. Try that again in a moment.';

  const lines = [`**${company} briefing**`, `- ${facts.join('\n- ')}`];
  if (top) lines.push(`\n**Best next move:** ${top.title}. ${top.detail}`);
  if (action) lines.push(`\n**${agent}:** Open ${action.label.replace(/^Open /, '')} to work on that now.`);
  else lines.push('\nNitro is using your verified workspace data for this briefing.');
  return lines.join('\n');
}

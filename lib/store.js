...
export async function logEmail({ to, subject, contactId, timestamp }) {
  const entry = { type: 'email', to, subject, contactId, timestamp: timestamp || Date.now(), replied: false, unsubscribed: false };
...
export async function logSms({ to, body, contactId, timestamp }) {
  const entry = { type: 'sms', to, body, contactId, timestamp: timestamp || Date.now(), replied: false };

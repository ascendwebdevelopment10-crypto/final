...
export async function logEmail({ to, subject, body, contactId, timestamp, segment, service }) {
    const entry = { type: 'email', to, subject, body: body || '', contactId, timestamp: timestamp || Date.now(), replied: false, unsubscribed: false, segment: segment || 'general', service: service || 'website' };
...
export async function logSms({ to, body, contactId, timestamp, segment, service }) {
    const entry = { type: 'sms', to, body, contactId, timestamp: timestamp || Date.now(), replied: false, segment: segment || 'general', service: service || 'website' };

...
export async function wasEmailed(email) {
  ...
  return await kv.sismember('email:contacted', String(email).toLowerCase());
}
export async function markEmailed(email) {
  if (!email) return;
  await kv.sadd('email:contacted', String(email).toLowerCase());
}

// Hunter queue dedup is separate from sent-email dedup: adding a recipient to a
// draft sequence must not be counted as a delivered email.
export async function wasHunterQueued(email) {
  if (!email) return false;
  return await kv.sismember('email:hunter_queued', String(email).toLowerCase());
}
export async function markHunterQueued(email) {
  if (!email) return;
  await kv.sadd('email:hunter_queued', String(email).toLowerCase());
}

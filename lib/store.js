...
export async function logEmail({ to, subject, body, contactName, contactId, timestamp, segment, service, id, providerId, status }) {
        const emailId = id || (Date.now() + '-' + Math.random().toString(36).slice(2, 8));
        const entry = { type: 'email', id: emailId, providerId: providerId || '', to, subject, body: body || '', contactName: contactName || '', contactId, timestamp: timestamp || Date.now(), status: status || 'sent', replied: false, unsubscribed: false, segment: segment || 'general', service: service || 'website' };

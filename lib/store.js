...
export async function logEmail({ to, subject, body, contactName, contactId, timestamp, segment, service }) {
      const entry = { type: 'email', to, subject, body: body || '', contactName: contactName || '', contactId, timestamp: timestamp || Date.now(), replied: false, unsubscribed: false, segment: segment || 'general', service: service || 'website' };

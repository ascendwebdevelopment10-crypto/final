...
export async function logEmail({ to, subject, body, contactName, contactId, timestamp, segment, service, id, providerId, status, businessLocation, businessWebsite, industry, targetSegment, followUpOf, sequence }) {
        const emailId = id || (Date.now() + '-' + Math.random().toString(36).slice(2, 8));
        const entry = {
                type: 'email', id: emailId, providerId: providerId || '', to, subject,
                body: body || '', contactName: contactName || '', contactId,
                timestamp: timestamp || Date.now(), status: status || 'sent',
                replied: false, unsubscribed: false, segment: segment || 'general',
                service: service || 'website', businessLocation: businessLocation || '',
                businessWebsite: businessWebsite || '', industry: industry || '',
                targetSegment: targetSegment || '',
                followUpOf: followUpOf || '', sequence: Number(sequence || 1),
        };
...
// Delivery events are kept separately from the immutable send log. Resend can
// replay webhooks, so storing the latest provider event by email id keeps the
// owner dashboard idempotent and avoids rewriting the whole list concurrently.
export async function recordEmailEvent({ providerId, to, status, timestamp, detail }) {
  const id = String(providerId || '').trim();
  if (!id) return;
  const event = {
    providerId: id,
    to: emailAddress(Array.isArray(to) ? to[0] : to),
    status: String(status || 'sent').replace(/^email\./, ''),
    timestamp: timestamp || Date.now(),
    detail: String(detail || '').slice(0, 500),
  };
  await kv.hset('outreach:email:events', { [id]: JSON.stringify(event) });
}

export async function getEmailEvents() {
  const raw = await kv.hgetall('outreach:email:events');
  const events = {};
  for (const [id, value] of Object.entries(raw || {})) {
    if (typeof value === 'object' && value) events[id] = value;
    else { try { events[id] = JSON.parse(value); } catch {} }
  }
  return events;
}

// Deliverability counters.
export async function logBounce(n) {
  await kv.incrby('stats:bounced', Number.isFinite(+n) ? +n : 1);
}
export async function logNotSent(n) {
  await kv.incrby('stats:not_sent', Number.isFinite(+n) ? +n : 1);
}

// Email dedup: remember every address we've emailed so we never contact the
// same business twice (mirrors the SMS sms:contacted_numbers logic).
export async function wasEmailed(email) {
  if (!email) return false;
  const normalized = String(email).toLowerCase();
  const [current, legacy] = await Promise.all([
    kv.sismember('email:contacted', normalized),
    kv.sismember('emailed:set', normalized),
  ]);
  return !!(current || legacy);
}
export async function markEmailed(email) {
  if (!email) return;
  const normalized = String(email).toLowerCase();
  await Promise.all([
    kv.sadd('email:contacted', normalized),
    kv.sadd('emailed:set', normalized),
  ]);
}
...
export async function logReply({ from, subject, body, timestamp, originalTo, thread, eventId }) {
      if (eventId) {
              const first = await kv.sadd('outreach:reply:events', String(eventId));
      ...
      const updated = log.map(e => emailAddress(e.to) === sender ? { ...e, replied: true } : e);
...
export async function getEmailLog(limit = 100) {
      const raw = await kv.lrange('email:log', 0, limit == null ? -1 : limit - 1);
      return raw.map(r => typeof r === 'string' ? JSON.parse(r) : r);
}
...
export async function addToSuppression(email) {
      const added = await kv.sadd('suppression:emails', email.toLowerCase());
      if (added) await kv.incr('stats:unsubscribed');
}
...
export async function listSuppressed() {
      return await kv.smembers('suppression:emails');
}

export async function getTotalStats() {
      const [total, replies, unsubscribed, smsTotal, smsReplies, callsMade, callsAnswered, followupSms, bounced, notSent] = await Promise.all([
              ...
              kv.get('stats:bounced'),
              kv.get('stats:not_sent')
            ]);
      	const today = mtDateStr();
      ...
      return {
              ...
              bounced: parseInt(bounced || 0),
              notSent: parseInt(notSent || 0),
              dailyChart: days
      };
}


export async function trackEmailOpen(id, { automated = false } = {}) {
        if (!id) return;
        await kv.hincrby('email:opens:count', id, 1);
        if (automated) await kv.hincrby('email:opens:automated', id, 1);
...
export async function trackEmailClick(id, url) {
        ...
        const existing = await kv.hget('email:clicks:first', id);
        if (!existing) await kv.hset('email:clicks:first', { [id]: Date.now() });
...
export async function getEmailEngagement() {
        const [opens, automatedOpens, clicks, opensFirst, opensLast, clicksFirst, clicksLast, clicksUrl] = await Promise.all([
                  kv.hgetall('email:opens:count'),
                  kv.hgetall('email:opens:automated'),
                  kv.hgetall('email:clicks:count'),
                  kv.hgetall('email:opens:first'),
                  kv.hgetall('email:opens:last'),
                  kv.hgetall('email:clicks:first'),
                  kv.hgetall('email:clicks:last'),
                  kv.hgetall('email:clicks:url')
                ]);
        return {
                  opens: opens || {}, automatedOpens: automatedOpens || {},
                  clicks: clicks || {},
                  opensFirst: opensFirst || {},
                  opensLast: opensLast || {},
                  clicksFirst: clicksFirst || {},
                  clicksLast: clicksLast || {},
                  clicksUrl: clicksUrl || {}
        };
}

// deploy nudge 20260716T230640Z

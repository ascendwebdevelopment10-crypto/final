...
const BOT_PATTERN = /(bot|crawler|spider|slurp|bingpreview|facebookexternalhit|linkedinbot|twitterbot|discordbot|headlesschrome|lighthouse|pagespeed|vercel-screenshot|google-inspectiontool|chrome-lighthouse|googleimageproxy|yahoomailproxy|proofpoint|mimecast|barracuda|urlscan|bytespider|semrush|ahrefs|mj12|uptimerobot|curl|wget|python-requests|node-fetch|axios|postmanruntime)/i;
...
export function isKnownAutomatedTraffic({ userAgent, purpose, webdriver, visibility, city } = {}) {
  return BOT_PATTERN.test(String(userAgent || ''))
    || webdriver === true
    || String(visibility || '').toLowerCase() === 'prerender'
    || /prefetch|prerender/i.test(String(purpose || ''))
    || DATA_CENTER_CITIES.has(normalizedCity(city));
}

...
const COOKIE = 'nitro_customer_session';
const SESSION_TTL = 30 * 24 * 60 * 60 * 1000;
const PASSWORD_ITERS = 210000;

function signingKey() {
  return 'nitro-customer-v1:' + (process.env.CUSTOMER_AUTH_SECRET || process.env.CRON_SECRET || '');
}
...
export function publicCustomer(user) {
  ...
  if (safe.meta) {
    const { token, ...publicMeta } = safe.meta;
    const expiresAt = Date.parse(publicMeta.tokenExpiresAt || '');
    const expired = Number.isFinite(expiresAt) && expiresAt <= Date.now();
    const healthy = Boolean(token && publicMeta.igUserId && !expired);
    safe.meta = {
      ...publicMeta,
      connected: healthy,
      connectionStatus: healthy ? 'connected' : expired ? 'expired' : 'broken',
    };
  }
  if (safe.socialConnections) {
    safe.socialConnections = Object.fromEntries(Object.entries(safe.socialConnections).map(([platform, connection]) => {
      const { accessToken, refreshToken, pageAccessToken, availablePages, ...publicConnection } = connection || {};
      const expiresAt = Date.parse(publicConnection.tokenExpiresAt || '');
      const expired = Number.isFinite(expiresAt) && expiresAt <= Date.now();
      const platformReady = platform === 'facebook'
        ? Boolean(publicConnection.pageId && pageAccessToken)
        : Boolean(publicConnection.accountId && accessToken);
      const healthy = Boolean(publicConnection.connected && platformReady && !expired);
      return [platform, {
        ...publicConnection,
        availablePages: platform === 'facebook' ? (availablePages || []).map(page => ({ id: page.id, name: page.name })) : undefined,
        connected: healthy,
        connectionStatus: healthy ? 'connected' : expired ? 'expired' : 'broken',
      }];
    }));

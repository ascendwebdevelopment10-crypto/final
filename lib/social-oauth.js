...
const FACEBOOK_REQUIRED_PERMISSIONS = ['pages_show_list', 'pages_read_engagement', 'pages_manage_posts'];
...
export function socialProviderStatus({ allowUnreleased = false } = {}) {
  return Object.fromEntries([...SUPPORTED].map(platform => {
    const creds = credentials(platform);
    const enabled = allowUnreleased || process.env[`SOCIAL_${platform.toUpperCase()}_ENABLED`] === 'true';
    return [platform, Boolean(enabled && creds.id && creds.secret && stateSecret())];
  }));
}

export function socialProviderAvailable(platform, options = {}) {
  const normalized = String(platform || '').toLowerCase();
  return Boolean(validSocialPlatform(normalized) && socialProviderStatus(options)[normalized]);
}
...
export function socialAuthorizationUrl(platform, state) {
  ...
  if (platform === 'tiktok') {
    const params = new URLSearchParams({ client_key: creds.id, redirect_uri: REDIRECT_URI, state, response_type: 'code', scope: 'user.info.basic,video.publish' });
...
async function jsonRequest(url, init) {
  ...
  const providerError = data.error && !(typeof data.error === 'object' && String(data.error.code || '').toLowerCase() === 'ok') ? data.error : null;
  if (!response.ok || providerError) {
    const error = new Error(data.error_description || providerError?.message || providerError?.code || providerError || `Social provider request failed (${response.status})`);
...
async function validateFacebookConnection(version, token, pages) {
  const permissions = await jsonRequest(`https://graph.facebook.com/${version}/me/permissions?access_token=${encodeURIComponent(token)}`);
  const granted = new Set((permissions.data || []).filter(item => item.status === 'granted').map(item => item.permission));
  const missing = FACEBOOK_REQUIRED_PERMISSIONS.filter(permission => !granted.has(permission));
  if (missing.length) throw new Error(`Facebook did not grant the required Page permissions: ${missing.join(', ')}.`);
  if (!(pages.data || []).length) throw new Error('Facebook connected, but no manageable Page was returned. Make sure your Facebook account has access to the Page you want Nitro to use.');
  return [...granted];
}
...
export async function completeSocialConnection(platform, code) {
  ...
  if (platform === 'facebook') {
    ...
    const longLived = await jsonRequest(`https://graph.facebook.com/${version}/oauth/access_token?${new URLSearchParams({ grant_type: 'fb_exchange_token', client_id: creds.id, client_secret: creds.secret, fb_exchange_token: tokenData.access_token })}`);
    tokenData = { ...tokenData, ...longLived };
    ...
    const grantedPermissions = await validateFacebookConnection(version, tokenData.access_token, pages);
    const page = pages.data?.[0] || null;
    profile = {
      accountId: member.id,
      accountName: page?.name || member.name || 'Facebook',
      pageId: page?.id || '',
      pageAccessToken: page?.access_token || '',
      availablePages: (pages.data || []).map(item => ({ id: item.id, name: item.name, accessToken: item.access_token })),
      grantedPermissions,
    };
    ...
    const accountId = user.data?.user?.open_id || tokenData.open_id || '';
    if (!accountId) throw new Error('TikTok authorized Nitro but did not return a usable account.');
    profile = { accountId, accountName: user.data?.user?.display_name || 'TikTok' };
    ...
    if (!user.sub) throw new Error('LinkedIn authorized Nitro but did not return a usable member account.');
    ...
    if (!item?.id) throw new Error('YouTube authorized Nitro but no YouTube channel was returned for this Google account.');
    profile = { accountId: item.id, accountName: item.snippet?.title || 'YouTube' };
  }

  if (!tokenData?.access_token) throw new Error(`${platform} did not return a usable access token.`);
...
export async function refreshSocialConnection(platform, connection) {
  if (!connection?.refreshToken) throw new Error(`Reconnect ${platform}; its authorization expired.`);
  const creds = credentials(platform);
  let tokenData;
  if (platform === 'tiktok') {
    tokenData = await jsonRequest('https://open.tiktokapis.com/v2/oauth/token/', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ client_key: creds.id, client_secret: creds.secret, grant_type: 'refresh_token', refresh_token: connection.refreshToken }) });
  } else if (platform === 'youtube') {
    tokenData = await jsonRequest('https://oauth2.googleapis.com/token', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ client_id: creds.id, client_secret: creds.secret, grant_type: 'refresh_token', refresh_token: connection.refreshToken }) });
  } else if (platform === 'linkedin') {
    tokenData = await jsonRequest('https://www.linkedin.com/oauth/v2/accessToken', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'refresh_token', refresh_token: connection.refreshToken, client_id: creds.id, client_secret: creds.secret }) });
  } else {
    throw new Error('Reconnect Facebook to renew Page publishing access.');
  }
  return {
    ...connection,
    connected: true,
    ...
    tokenExpiresAt: tokenData.expires_in ? new Date(Date.now() + Number(tokenData.expires_in) * 1000).toISOString() : connection.tokenExpiresAt,

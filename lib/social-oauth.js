...
async function jsonRequest(url, init) {
  ...
  const providerError = data.error && !(typeof data.error === 'object' && String(data.error.code || '').toLowerCase() === 'ok')
    ? data.error
    : null;
  if (!response.ok || providerError) {
    throw new Error(data.error_description || providerError?.message || providerError?.code || providerError || `Social provider request failed (${response.status})`);
  }
  return data;
}
...
export async function completeSocialConnection(platform, code) {
  ...
  if (platform === 'facebook') {
    ...
    profile = { accountId: member.id, accountName: page?.name || member.name || 'Facebook', pageId: page?.id || '', pageAccessToken: page?.access_token || '', availablePages: (pages.data || []).map(item => ({ id: item.id, name: item.name, accessToken: item.access_token })) };

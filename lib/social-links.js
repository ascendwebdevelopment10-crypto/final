...
export function socialAttributionFromRequest(req) {
  const cookies = String(req?.headers?.cookie || '').split(';');
  const row = cookies.find(value => value.trim().startsWith('nitro_social_attribution='));
  if (!row) return null;
  const token = decodeURIComponent(row.trim().slice('nitro_social_attribution='.length));
  return parseSocialLinkToken(token);
}

...
const NON_CONTACT_LOCALS = new Set([
  'noreply','no-reply','no_reply','donotreply','do-not-reply','do_not_reply',
  'mailer-daemon','postmaster','privacy','legal','abuse','security','webmaster',
  'accounting','billing','careers','feedback','hr','investors','jobs','media','press','wholesale',
]);
...
export function isLikelyRealEmail(raw) {
  ...
  if (NON_CONTACT_LOCALS.has(local)) return false;
  if (/^[a-f0-9]{24,}$/.test(local)) return false;     // telemetry/error-reporting token

...
async function getInsightMetrics(mediaId, token, metrics) {
  const response = await getJson(`${IG_GRAPH}/${mediaId}/insights?metric=${metrics.join(',')}&access_token=${encodeURIComponent(token)}`);
...
async function getMediaInsights(mediaId, token) {
  ...
  const errors = [];
  try {
    data = await getInsightMetrics(mediaId, token, primary);
  } catch (groupError) {
    // Meta rejects an entire multi-metric request when only one metric is not
    // supported for a media type. Fetching them separately keeps the valid
    // values instead of turning the whole post into an empty result.
    const results = await Promise.all(primary.map(async metric => {
      try { return await getInsightMetrics(mediaId, token, [metric]); }
      catch (error) { errors.push(error); return []; }
    }));
    data = results.flat();
  }

  // Older Reels and older API versions can expose the visible play/view count
  // under plays or impressions. Only ask for those aliases when views is absent
  // or zero, and keep the largest valid value so Nitro never under-reports it.
  let legacyViews = [];
  const currentViews = insightValue(data, 'views');
  if (currentViews == null || currentViews === 0) {
    const aliases = await Promise.all(['plays', 'impressions'].map(async metric => {
      try { return await getInsightMetrics(mediaId, token, [metric]); }
      catch (error) { errors.push(error); return []; }
    }));
    legacyViews = aliases.flat();
  }
  const viewCandidates = [
    insightValue(data, 'views'),
    insightValue(legacyViews, 'plays'),
    insightValue(legacyViews, 'impressions'),
  ].filter(Number.isFinite);
  if (!data.length && errors.length) throw errors[0];
  return {
    views: viewCandidates.length ? Math.max(...viewCandidates) : null,
    reach: insightValue(data, 'reach'),
    saved: insightValue(data, 'saved'),
    shares: insightValue(data, 'shares'),
    interactions: insightValue(data, 'total_interactions'),
  };
}

...
export const OUTREACH_OSM_TAGS = [
  ...
  'craft=painter', 'craft=gardener', 'craft=carpenter', 'craft=locksmith',
];
...
async function overpassQuery(body) {
  ...
  // One bounded pass keeps a slow public mirror from consuming the entire
  // five-minute cron window. The next scheduled run rotates metros and tries again.
  for (let attempt = 0; attempt < OVERPASS_ATTEMPTS; attempt++) {
    ...
    if (attempt + 1 < OVERPASS_ATTEMPTS) await sleep(1500 + Math.random() * 1500);
  }
  return null;
}

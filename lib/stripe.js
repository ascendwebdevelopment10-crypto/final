...
export async function createContentCreditCheckoutSession({ customerId, email, stripeCustomerId, credits, amountCents, successUrl, cancelUrl }) {
  const params = {
    mode: 'payment',
    'payment_method_types[0]': 'card',
    'line_items[0][quantity]': '1',
    'line_items[0][price_data][currency]': 'usd',
    'line_items[0][price_data][product_data][name]': `Nitro Content Credits — ${credits} credits`,
    'line_items[0][price_data][unit_amount]': String(amountCents),
    success_url: successUrl,
    cancel_url: cancelUrl,
    client_reference_id: customerId,
    'metadata[purchaseType]': 'content_credits',
    'metadata[customerId]': customerId,
...
// Compatibility for old imports while active clients move to Content credits.
export const createVideoCreditCheckoutSession = createContentCreditCheckoutSession;

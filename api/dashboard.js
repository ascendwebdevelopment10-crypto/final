...
const emailsHtml = '<div class="page" id="page-emails"><div class="section"><h2>Email Log <span style="font-size:12px;color:#94a3b8;font-weight:400">&mdash; click any row to see full message</span></h2><table><thead><tr><th>To</th><th>Subject</th><th>Service</th><th>Segment</th><th>Time</th><th>Status</th></tr></thead><tbody id="email-tbody"></tbody></table></div></div>';

const smsHtml = '<div class="page" id="page-sms"><div class="section"><h2>SMS Log <span style="font-size:12px;color:#94a3b8;font-weight:400">&mdash; click any row to see full message</span></h2><table><thead><tr><th>To</th><th>Message Preview</th><th>Service</th><th>Segment</th><th>Time</th><th>Status</th></tr></thead><tbody id="sms-tbody"></tbody></table></div></div>';
...
if(dr){
var xid=dr.dataset.xid;
if(!xid)return;
var xr=document.getElementById(xid);
if(!xr)return;
xr.classList.toggle('open');
dr.classList.toggle('row-open');
}
});
...
return '<span class="bweb">Website</span>';
}
...
var rawBody=(e.body||'').replace(/<[^>]+>/g,'').trim();
var fullBody=esc(rawBody||'(email body not stored for this entry — visible in future sends)');
rows+='<tr class="data-row" data-xid="'+xid+'"><td>'+esc(e.to||'-')+'</td><td>'+subj+'<span class="expand-indicator">&#9658;</span></td><td>'+svcBadge(e.service)+'</td><td>'+(sl[e.segment]||e.segment||'-')+'</td><td>'+fmt(e.timestamp)+'</td><td><span class="'+(e.replied?'br':'bs')+'">'+(e.replied?'Replied':'Sent')+'</span></td></tr>';
rows+='<tr class="expand-row" id="'+xid+'"><td colspan="6"><div class="msg-label">Full Email Body</div><div class="msg-full">'+fullBody+'</div></td></tr>';
});
...
var xid='xsms'+i;
var rawBody=(e.body||'').trim();
var preview=esc(rawBody.substring(0,60))+(rawBody.length>60?'...':'');
var fullMsg=esc(rawBody||'(no message)');
rows+='<tr class="data-row" data-xid="'+xid+'"><td>'+esc(e.to||'-')+'</td><td>'+preview+'<span class="expand-indicator">&#9658;</span></td><td>'+svcBadge(e.service)+'</td><td>'+(sl[e.segment]||e.segment||'-')+'</td><td>'+fmt(e.timestamp)+'</td><td><span class="'+(e.replied?'br':'bsm')+'">'+(e.replied?'Replied':'Sent')+'</span></td></tr>';
rows+='<tr class="expand-row" id="'+xid+'"><td colspan="6"><div class="msg-label">Full SMS Message</div><div class="msg-full">'+fullMsg+'</div></td></tr>';
});

...
// Fixed pitch sent when a real person replies
function pitchMessage(service) {
...
export default async function handler(req, res) {
  ...
  if (!alreadyAutoReplied && !isNotInterested(body)) {
        try {
                const replyBody = isAutomatedReply(body) ? AUTOMATED_REPLY_TEXT : pitchMessage(originalService);

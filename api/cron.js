...
async function processEmailContact(contact, segment, emailsSent) {
        try {
...
async function processSmsContact(contact, segment) {
        try {
                  if (!contact.phone) return null;
                  const { text, service } = await generateSms(contact, segment);
                  await twilioClient.messages.create({ body: text, from: TWILIO_FROM, to: contact.phone });
                  await logSms({ to: contact.phone, body: text, contactName: contact.organization_name, timestamp: Date.now(), segment, service });
                  return 'sms';
        } catch(e) { return { error: e.message }; }
}
...
export default async function handler(req, res) {
  ...
  try {
            // Fetch 3 query batches in parallel
          ...
          // Scrape emails in parallel (3s timeout each)

...
async function generateSms(contact, segment) {
  ...
  let text = cleanPlaceholders(msg.content[0].text.trim().replace(/^["']|["']$/g, '').trim());
      if (!text.includes(WEBSITE_URL)) {
              text = text.replace(/- Ascend Web Dev\s*$/i, `- Ascend Web Dev | ${WEBSITE_URL}`);
              if (!text.includes(WEBSITE_URL)) text = text + ` - Ascend Web Dev | ${WEBSITE_URL}`;
      }
      return { text, service };
}

async function generateFollowUpSms(contact, originalBody) {
      const firstName = contact.first_name || 'there';
      const company = contact.organization_name || 'your business';
      const prompt = `Write a very short, casual follow-up SMS (under 200 characters including sign-off) to ${firstName} at ${company}. We sent them a cold SMS a few days ago about web/digital services from Ascend Web Development and never heard back. Keep it light - just checking in, no pressure. Don't repeat the pitch. End with a soft question. Sign off: - Ascend Web Dev | ${WEBSITE_URL}. No emojis. Output ONLY the SMS text.`;
      const msg = await anthropic.messages.create({
              model: ANTHROPIC_MODEL,
              max_tokens: 150,
              messages: [{ role: 'user', content: prompt }]
...
export default async function handler(req, res) {
  ...
  try {
          // ── FOLLOW-UP: send to leads that got an SMS 3 days ago with no reply ──
        try {
            ...
            for (const lead of followupCandidates) {
                        try {
                                      ...
                                      const followUpText = await generateFollowUpSms(contactObj, lead.body);
        ...
        // Normalize, deduplicate by phone, filter toll-free numbers

...
async function fetchOutscraperLeads(query, limit = 50) {
...
export default async function handler(req, res) {
  ...
  try {
            ...
            const [batch1, batch2] = await Promise.all([
                          fetchOutscraperLeads(primaryQuery, 50),
                          fetchOutscraperLeads(secondaryQuery, 50)
                        ]);

            const allLeads = [...batch1, ...batch2].map(normalizeContact);
              const noWebLeads = allLeads.filter(c => hasNoWebsite(c));
              const appLeads = allLeads.filter(c => !hasNoWebsite(c));

...
export async function runSocialPublish() {
  ...
  for (const key of keys) {
    ...
    const unsafeGenerated = posts.filter(p => p.autoWeek === true && p.status === 'scheduled' && p.scheduledFor && Date.parse(p.scheduledFor) <= now && generatedCaptionNeedsReview(`${p.title || ''} ${p.text || ''}`));
    for (const post of unsafeGenerated) { post.status = 'failed'; post.error = 'Generated caption needs review before publishing.'; post.failedAt = new Date().toISOString(); changed = true; errors.push({ user: user.id, platform: post.platform || 'instagram', error: post.error }); }

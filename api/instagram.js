...
const PROFILE_DEFAULTS = {
  displayName: 'Nitro Outreach | AI Growth System',
  handle: '@nitrooutreach',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'callback') {
      ...
      const username = account.username || 'nitrooutreach';
    ...
    if (action === 'publish') {
      ...
      if (!sameHost(req, mediaUrl)) { res.status(400).json({ error: 'Only media generated by this Nitro app can be published' }); return; }
